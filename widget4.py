...
def register_callbacks(app):
    ...
    def update_indicator(faculty_name):
        ...
        if not faculty_name:
            return html.I("Please select a faculty member", style=style)
        else:
            return html.I(f"Faculty: {faculty_name}", style=style)
    ...
    def update_profile(
        faculty_name, edit_clicks, update_clicks, research_signal,
        edit_title, edit_phone, edit_email, edit_ri
    ):
        ...
        if triggered == "w4-update-btn" and faculty_name:
            ...
            cursor.execute(
                "UPDATE faculty SET position=%s, phone=%s, email=%s, research_interest=%s WHERE name=%s",
        ...
        if faculty_name:
            conn, cursor = get_mysql_cursor()
            cursor.execute(
                "SELECT f.position, f.phone, f.email, f.research_interest, f.photo_url, u.name FROM faculty f JOIN university u ON f.university_id = u.id WHERE f.name = %s",
                (faculty_name,)
            )
            ...
            cursor.execute(
                "SELECT p.title, p.year, p.num_citations FROM faculty f JOIN faculty_publication fp ON f.id = fp.faculty_id JOIN publication p ON fp.publication_id = p.id WHERE f.name = %s ORDER BY p.num_citations DESC",
                (faculty_name,)
            )
            ...
            if pubs:
                ...
                top_paper_title = pubs[0][0]
                top_paper_year = pubs[0][1]
            else:
                total_citations = 0
            cursor.close()
            conn.close()
        ...
        if faculty_name:
            ...
            view_content = [
                html.Div("Please select a faculty member.", style={"color": "#999"})
            ]

        if photo_url:
            photo = html.Img(src=photo_url, style={
                "width": "84px", "height": "84px", "objectFit": "cover",
                "borderRadius": "14px", "background": "#eee", "border": "1px solid #ccc"
            })
        else:
        ...
        if show_edit:
            edit_field_style = {"display": "block", "marginTop": "10px"}
        else:
            edit_field_style = {"display": "none"}

        view_container_style = {
            "background": "#fff", "borderRadius": "12px", "padding": "16px",
            "height": "208px", "overflowY": "auto", "boxSizing": "border-box",
            "boxShadow": "0 1px 5px #0001", "flex": "1",
            "display": "flex", "flexDirection": "column"
        }

        if show_edit:
            edit_btn_style = {"display": "none"}
            update_btn_style = {"display": "inline-block"}
        else:
            edit_btn_style = {"marginRight": "10px"}
            update_btn_style = {"display": "none"}

        if faculty_name:
            title_value = position
            phone_value = phone
            email_value = email
            ri_value = research_interest
        else:
            title_value = ""
            phone_value = ""
            email_value = ""
        ...
        return (
            photo, view_content, view_container_style, edit_field_style,
            title_value, phone_value, email_value, ri_value,
            edit_btn_style, update_btn_style, signal_value
        )

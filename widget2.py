...
def register_callbacks(app):
    ...
    def update_graph(professor, clear_clicks, keyword, university, professor_again):
        ...
        if filters:
            ...
            filter_text = "Showing all publications."
        ...
        if not (professor or university or keyword):
            ...
            sql_parts = []
            sql_parts.append("SELECT p.year, COUNT(p.id) AS num_publications")
            sql_parts.append("FROM publication p")
            sql_parts.append("JOIN faculty_publication fp ON p.id = fp.publication_id")
            sql_parts.append("JOIN faculty f ON fp.faculty_id = f.id")

            params = []
            if university:
                sql_parts.append("JOIN university u ON f.university_id = u.id")
            if keyword:
                sql_parts.append("JOIN publication_keyword pk ON p.id = pk.publication_id")
                sql_parts.append("JOIN keyword k ON pk.keyword_id = k.id")
        ...
        if not rows:
            ...
            return filter_text, go.Figure(layout=layout)
